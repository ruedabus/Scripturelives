...
export async function generateMetadata({
  ...
  if (!post) return { title: "Artículo No Encontrado | Scripture Lives" };
...
function RelatedCard({ post }: { post: BlogPostES }) {
  const col = CATEGORY_COLORS_ES[post.category];
  return (
    <Link
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-gray-400 mt-0.5">{post.readingTimeMin} min de lectura</p>
      </div>
    </Link>
  );
}
...
// ── Page ─────────────────────────────────────────────────────────────────────
export default async function DevotionalArticleESPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const related = getRelatedPostsES(slug, 3);
  const col = CATEGORY_COLORS_ES[post.category];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-gradient-to-b from-white to-amber-50 border-b border-amber-100">
        <div className="max-w-3xl mx-auto px-4 py-12 text-center">
          ...
          <div className="flex items-center justify-center gap-2 mb-4 flex-wrap">
            ...
            <span className="text-xs text-gray-400">{post.readingTimeMin} min de lectura</span>
      ...
      <main className="max-w-3xl mx-auto px-4 pb-12">
        ...
        <div className="mt-8 flex items-center justify-between flex-wrap gap-3 rounded-xl bg-white border border-gray-100 px-5 py-4">
          <div>
            <p className="text-xs text-gray-400">Escrito por</p>
      ...
      <footer className="border-t border-gray-200 bg-amber-50 px-4 py-10 text-center">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-2">¿Listo para profundizar más?</h3>
        <p className="text-sm text-gray-600 mb-4 max-w-sm mx-auto">
          Usa el Lector Bíblico de Scripture Lives para explorar cualquier pasaje, comparar
          traducciones y estudiar el griego y el hebreo original.
        </p>
        <Link
          href="/bible"
