...
export function generateBracket(
  ...
  // Subsequent rounds — player IDs unknown until earlier rounds complete
  ...
  for (let r = 2; r <= rounds; r++) {
    ...
    for (let i = 0; i < prevRoundCount; i++) {
      matches.push(makeBracketMatch({
        ...
        player2Id: null,
        isBye: false,
        questionsPerMatch: r === rounds ? Math.min(questionsPerMatch + 2, 7) : questionsPerMatch,
        difficulties,
      }));
    }
  }
...
function makeBracketMatch({
  round, slot, player1Id, player2Id, isBye, questionsPerMatch, difficulties,
}: {
  round: number;
  slot: number;
  ...
  return {
    ...
    loserId:      isBye ? null : null,
    isBye,
    questions:    isBye ? [] : pickQuestions(questionsPerMatch, difficulties),
    answers:      {},
    matchScore:   {},
...
export function advanceWinner(matches: BracketMatch[], completedMatch: BracketMatch): BracketMatch[] {
  ...
  return matches.map((m) => {
    ...
    // slot 0 of a pair → player1 slot; slot 1 → player2 slot
...
/**
 * Get the current active match (first pending match with both players filled).
 */
...
export function pickQuestions(
  ...
  const pool = (customPool ?? TOURNAMENT_QUESTIONS).filter(
